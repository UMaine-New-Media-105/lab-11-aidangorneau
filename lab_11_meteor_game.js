...
/*
TODO
1. make more detailed ship sprite(add a window, some boosters on the back, small triangle wings on the side)
2. make the ship lose health or die when collide with meteor
3. add a score to track number of meteors hit with laser
...
function draw{
  ship.move
  ship.show
  loop through meteors and update and draw them
  loop through lasers and update and draw them only if SPACE_BAR is pressed
  loop through meteors, if laser collides then make new small meteors
    //if laser collides with meteor, increase score 
  //if ship collides with meteor, decrease health or kill ship
}
...
class ship{
  ...
  //make ship change sides of the canvas seamlessly by changing its axis by the width of the canvas once it is completely off one side of the screen
  }
  show(){
  push();
...
class meteor{
  ...
  this.x +=this.x;
  this.y +=this.y;
  //make meteors change sides of the canvas seamlessly by changing its axis by the width of the canvas once it is completely off one side of the screen
  }
  show(){
  push();
  translate(this.x, this.y);
  scale();
  noFill();
  //make random shapes with random 5 to 10 sides using beginShape and endShape with random locations for each point
  pop()
  }
}
...
class laser{
  ...
  this.x +=this.x;
  this.y +=this.y;
  //set starting position to x and y of the ship, if SPACE_BAR is pressed move in the direction the ship is pointing
  //if hit meteor while moving, return laserCollisionDetection to be true
  }
  show(){
  push();
  translate(this.x, this.y);
...
function shipCollisionDetection(ship, meteor){
  let shipHit = dist(ship.x, ship.y, meteor.x, meteor.y);
  if (shipHit < shipWidth) {
    return true;
  } else {
    return false;
  }
...
function laserCollisionDetection(laser, meteor){
  let laserHit = dist(laser.x, laser.y, meteor.x, meteor.y);
  if (laserHit < laserWidth) {
    return true;
  } else {
    return false;
  }
...
function keyPressed
if UP_ARROW pressed, increase ship forward speed
if DOWN_ARROW pressed, decrease ship forward speed
if RIGHT_ARROW pressed, rotate ship clockwise
if LEFT_ARROW pressed, rotate ship counter-clockwise
if SPACE_BAR pressed, set laser to true
